...
export default function ThirdPersona() {
  ...
  const [expanded, setExpanded] = useState({});   // inline receipts, per pattern id
  ...
  // Reconsider: demo-mode only. The real backend has no un-confirm path —
  // by design, decisions about the self are weighty. In the demo we allow
  // exploration.
  const reconsider = (p) => {
    if (live) return;
    setPatterns((ps) => ps.map((x) => x.id === p.id ? { ...x, status: "hypothesis", confirmedOn: undefined } : x));
    setRejections((r) => r.filter((x) => x.id !== p.id));
  };
  ...
  // ===== APP — the designed surface =====
  // Layout harvested from the dc prototype: the self is CONSTANT (left,
  // sticky), and everything the product does happens TO it (right, acts).
  // Machinery is v9's: live backend mode, real lifecycle, circuit breaker.

  const ActHeader = ({ eyebrow, title, lede }) => (
    <div style={{ marginBottom: 24 }}>
      <div style={{ fontSize: 11, letterSpacing: "0.24em", textTransform: "uppercase", color: T.gold, fontWeight: 600 }}>{eyebrow}</div>
      <h2 style={{ ...S.display, fontSize: isNarrow ? 26 : 34, lineHeight: 1.15, margin: "12px 0 0", background: `linear-gradient(135deg, ${T.ink} 40%, ${T.goldBright} 100%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{title}</h2>
      <p style={{ fontSize: 15, color: T.ink2, lineHeight: 1.8, maxWidth: "58ch", margin: "14px 0 0", fontFamily: T.serif }}>{lede}</p>
    </div>
  );

  const Receipts = ({ p }) => (
    <div style={{ marginTop: 14, borderTop: `1px solid ${T.border}`, paddingTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: T.ink3 }}>The receipts. Only you see these.</div>
      {(p.evidence || []).map((sid) => {
        const e = entries.find((x) => x.id === sid);
        if (!e) return null;
        return (
          <div key={sid} style={{ background: T.well, border: `1px solid ${T.borderSoft}`, borderRadius: 10, padding: "11px 13px" }}>
            <p style={{ margin: "0 0 6px", fontSize: 13, lineHeight: 1.65, color: "#C9D1D9", fontFamily: T.serif }}>"{e.text}"</p>
            <div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" }}>
              {e.people.map((pp, i) => <span key={i} style={S.badge(T.blueBright)}>{pp}</span>)}
              <span style={{ fontSize: 10, color: T.ink4, marginLeft: "auto" }}>{e.date}</span>
            </div>
          </div>
        );
      })}
      {(p.evidence || []).every((sid) => !entries.find((x) => x.id === sid)) && (
        <div style={{ fontSize: 12, color: T.ink3 }}>{p.evidence.length} linked {p.evidence.length === 1 ? "entry" : "entries"}. Connect live mode to read them.</div>
      )}
    </div>
  );

  const evidenceMeta = (p) => (
    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, color: T.ink3, flexWrap: "wrap" }}>
      <EvidenceDots count={p.evidence.length} />
      <span>{p.evidence.length} {p.evidence.length === 1 ? "entry" : "entries"}{p.spreadDays > 0 ? ` · across ${p.spreadDays} days` : ""}</span>
      <button onClick={() => setExpanded((x) => ({ ...x, [p.id]: !x[p.id] }))} style={{ marginLeft: 4, background: "none", border: "none", color: T.blueBright, font: "inherit", fontSize: 11, cursor: "pointer", textDecoration: "underline", padding: 0 }}>
        {expanded[p.id] ? "hide receipts" : `receipts (${p.evidence.length})`}
      </button>
    </div>
  );

  const previewTag = (
    <div style={{ marginBottom: 16, display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 20, background: "rgba(201,169,110,0.08)", border: "1px solid rgba(201,169,110,0.2)" }}>
  ...
  return (
    <div style={S.app}>
      {/* header */}
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "18px 24px 0", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 25, height: 25, borderRadius: "50%", background: `radial-gradient(circle at 35% 35%, ${T.goldBright}, ${T.blue})`, boxShadow: "0 0 16px rgba(201,169,110,0.4)" }} />
          <span style={{ fontSize: 11, letterSpacing: "0.28em", textTransform: "uppercase", color: T.gold, fontWeight: 700 }}>ThirdPersona</span>
          {live && <span style={{ fontSize: 10, color: T.green }}>● live</span>}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {!isNarrow && <span style={{ fontSize: 11.5, color: T.ink3 }}>private by architecture · nothing without receipts · you are the ground truth</span>}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "22px 24px 80px", display: "grid", gridTemplateColumns: isNarrow ? "1fr" : "404px 1fr", gap: 30, alignItems: "start" }}>

        {/* ══ LEFT: the self, constant ══ */}
        <aside style={{ position: isNarrow ? "static" : "sticky", top: 22, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ borderRadius: 20, overflow: "hidden", background: "rgba(10,13,20,0.5)", border: `1px solid ${T.border}`, backdropFilter: "blur(10px)" }}>
            <HumanMesh coverage={totalCov / 100} height={isNarrow ? 260 : 330} caption={false} pulse={pulse} />
            <div style={{ padding: "0 22px 20px", textAlign: "center" }}>
              <div style={{ ...S.display, fontSize: 40, lineHeight: 1, background: `linear-gradient(135deg, ${T.ink} 30%, ${T.goldBright} 100%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{totalCov}%</div>
              <div style={{ fontSize: 12.5, color: T.ink2, marginTop: 6 }}>
                of you exists here
                {justSaved && <span style={{ color: T.green, marginLeft: 6 }}>+ new pieces arriving</span>}
              </div>
              <div style={{ fontSize: 11, color: T.ink4, marginTop: 4, lineHeight: 1.5 }}>the dust around you is everything you haven't written yet</div>
              <div style={{ height: 3, borderRadius: 99, background: "rgba(255,255,255,0.06)", marginTop: 14, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${totalCov}%`, background: `linear-gradient(90deg, ${T.blueBright}, ${T.goldBright})`, borderRadius: 99, transition: "width .6s cubic-bezier(.2,.7,.3,1)" }} />
              </div>
            </div>
          </div>

          {/* say one true thing */}
          <div style={{ borderRadius: 16, background: T.glass, border: `1px solid ${T.border}`, padding: 16, backdropFilter: "blur(10px)" }}>
            <div style={{ ...S.eyebrow, marginBottom: 10 }}>Say one true thing</div>
            <textarea value={entryText} onChange={(e) => setEntryText(e.target.value)} placeholder="What happened? How did it actually feel?" style={{ width: "100%", minHeight: 74, background: T.well, border: `1px solid ${T.border}`, borderRadius: 10, padding: 12, color: T.ink, fontSize: 13.5, fontFamily: T.serif, lineHeight: 1.6, resize: "vertical", outline: "none", boxSizing: "border-box" }} />
            {entryText.length > 10 && (
              <div style={{ marginTop: 12, padding: 12, background: "rgba(7,9,14,0.6)", borderRadius: 10, border: `1px solid ${T.borderSoft}` }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 12, alignItems: "center" }}>
              ...
              <input value={pTag} onChange={(e) => setPTag(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && pTag.trim()) { setPeople([...people, pTag.trim()]); setPTag(""); } }} placeholder="who was there?" style={{ background: T.well, border: `1px solid ${T.border}`, borderRadius: 20, padding: "4px 12px", color: T.ink, fontSize: 11, outline: "none", width: 110, fontFamily: "inherit" }} />
            </div>
            <button onClick={addEntry} style={{ ...S.btnGold, marginTop: 12 }}>Add to yourself</button>
          </div>

          {/* still dust */}
          {gaps.some((d) => d.cov < 60) && (
            <div style={{ borderRadius: 16, background: "rgba(14,18,25,0.4)", border: "1px solid rgba(201,169,110,0.14)", padding: 16 }}>
              <div style={{ ...S.eyebrow, marginBottom: 12 }}>Still dust. Unwritten parts of you</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                {gaps.filter((d) => d.cov < 60).map((d) => (
                  <div key={d.id} style={{ padding: "10px 13px", background: T.well, borderRadius: 10, border: `1px solid ${T.borderSoft}` }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "#DAE2EC" }}>{d.label} <span style={{ color: T.ink4, fontWeight: 400 }}>· {d.cov}%</span></div>
                    <div style={{ fontSize: 11.5, color: T.ink2, marginTop: 3, fontStyle: "italic", fontFamily: T.serif }}>"{GAP_PROMPTS[d.id]}"</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* ══ RIGHT: what a built self unlocks ══ */}
        <section style={{ minWidth: 0 }}>
          <div style={{ display: "flex", gap: 6, borderBottom: `1px solid ${T.border}`, marginBottom: 24, overflowX: "auto" }}>
            {[["mirror","The Mirror"],["circle","Circle"],["dialogue","Dialogue"],["portable","Portable You"]].map(([k, l]) => (
              <button key={k} onClick={() => setTab(k)} style={{ padding: "11px 15px", background: "none", border: "none", borderBottom: `2px solid ${tab === k ? T.gold : "transparent"}`, color: tab === k ? T.ink : T.ink3, fontWeight: tab === k ? 600 : 400, fontSize: 13, fontFamily: "inherit", cursor: "pointer", whiteSpace: "nowrap", transition: "color .2s" }}>
                {l}
                {k === "mirror" && hypotheses.length > 0 && <span style={{ fontSize: 9, background: T.goldBright, color: "#0A0C12", borderRadius: 8, padding: "1px 6px", marginLeft: 6, fontWeight: 700 }}>{hypotheses.length}</span>}
                {k === "dialogue" && <span style={{ fontSize: 8, color: T.goldBright, marginLeft: 5, verticalAlign: "top" }}>✦</span>}
              </button>
            ))}
          </div>
          ...
          {tab === "mirror" && (<div>
            <ActHeader
              eyebrow="It notices. It never decides."
              title="Hypotheses, never verdicts."
              lede="Patterns surface only when the evidence exists. Three independent entries, minimum, each one traceable to your own words. Nothing becomes part of you without your yes. Enforced by a database trigger, not by good intentions."
            />

            <div style={{ ...S.card, padding: "12px 16px", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
              <span style={{ fontSize: 11, color: T.ink3 }}>rejection rate</span>
              <div style={{ flex: 1, minWidth: 120, height: 4, background: "#1A2130", borderRadius: 4, overflow: "hidden" }}>
              ...
              <span style={{ fontSize: 11, color: breakerTripped ? T.red : T.ink2 }}>{Math.round(rejRate * 100)}% / 40%</span>
            </div>
            {breakerTripped && (
              <div style={{ ...S.card, border: `1px solid ${T.red}44`, background: "rgba(240,126,112,0.06)" }}>
                ...
                <div style={{ fontSize: 12, color: T.ink2, lineHeight: 1.6 }}>Too many recent hypotheses missed. The system recalibrates from your rejections rather than guessing worse. This is by design.</div>
              </div>
            )}

            <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 14 }}>
              {hypotheses.map((p) => (
                <div key={p.id} style={{ ...S.card, marginBottom: 0, borderLeft: `3px solid ${CAT_COL[p.category] || T.gold}`, padding: "20px 22px", ...(flashId === p.id ? { boxShadow: `0 0 30px ${T.goldBright}44` } : {}) }}>
                  <div style={{ ...S.eyebrow, color: CAT_COL[p.category] || T.gold, marginBottom: 10 }}>We noticed</div>
                  <p style={{ margin: "0 0 12px", fontSize: isNarrow ? 15.5 : 17.5, lineHeight: 1.65, fontFamily: T.serif, color: T.ink }}>{p.insight.replace(/^We noticed( that)?\s*/i, "")}</p>
                  {evidenceMeta(p)}
                  {expanded[p.id] && <Receipts p={p} />}
                  {rejectingId === p.id ? (
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginTop: 16 }}>
                      <input autoFocus value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} placeholder="Why doesn't this fit? It teaches the system." style={{ flex: 1, minWidth: 200, background: T.well, border: `1px solid ${T.border}`, borderRadius: 8, padding: "8px 12px", color: T.ink, fontSize: 12, outline: "none", fontFamily: "inherit" }} />
                    ...
                    <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                      <button onClick={() => confirmPattern(p)} style={{ ...S.btnGold, padding: "11px 26px", fontSize: 13 }}>That's me</button>
                      <button onClick={() => setRejectingId(p.id)} style={{ ...S.btnGhost, padding: "11px 22px", fontSize: 13 }}>Not me</button>
                    </div>
                  )}
                </div>
              ))}
              ...
              {actives.map((p) => (
                <div key={p.id} style={{ ...S.card, marginBottom: 0, borderLeft: `3px solid ${T.green}`, padding: "20px 22px", boxShadow: "0 10px 40px rgba(84,196,142,0.10)", ...(flashId === p.id ? { boxShadow: `0 0 30px ${T.goldBright}44` } : {}) }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    <div style={{ ...S.eyebrow, color: CAT_COL[p.category] || T.gold }}>We noticed</div>
                    <span style={{ fontSize: 10, padding: "3px 10px", borderRadius: 99, background: "rgba(84,196,142,0.13)", color: T.green }}>part of you now{p.confirmedOn ? ` · ${p.confirmedOn}` : ""}</span>
                  </div>
                  <p style={{ margin: "0 0 12px", fontSize: 15, lineHeight: 1.65, fontFamily: T.serif, color: "#DAE2EC" }}>{p.insight.replace(/^We noticed( that)?\s*/i, "")}</p>
                  {evidenceMeta(p)}
                  {expanded[p.id] && <Receipts p={p} />}
                  {!live && <button onClick={() => reconsider(p)} style={{ marginTop: 12, background: "none", border: "none", color: T.ink3, font: "inherit", fontSize: 12, cursor: "pointer", padding: 0 }}>← reconsider</button>}
                </div>
              ))}
              ...
              {rejectedPs.map((p) => {
                ...
                return (
                  <div key={p.id} style={{ ...S.card, marginBottom: 0, opacity: 0.5, borderLeft: `3px solid ${T.ink4}`, padding: "18px 22px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <div style={{ ...S.eyebrow, color: T.ink4 }}>We noticed</div>
                      <span style={{ fontSize: 10, padding: "3px 10px", borderRadius: 99, background: "rgba(123,142,200,0.1)", color: T.ink3 }}>set aside</span>
                    </div>
                    <p style={{ margin: "0 0 8px", fontSize: 13, lineHeight: 1.65, color: T.ink3, textDecoration: "line-through", textDecorationColor: "rgba(154,166,181,0.35)" }}>{p.insight.replace(/^We noticed( that)?\s*/i, "")}</p>
                    <div style={{ fontSize: 11, color: T.ink4 }}>You said: "{(rej && rej.reason) || "not me"}". The system learned, and won't rediscover it.</div>
                    {!live && <button onClick={() => reconsider(p)} style={{ marginTop: 10, background: "none", border: "none", color: T.ink3, font: "inherit", fontSize: 12, cursor: "pointer", padding: 0 }}>← reconsider</button>}
                  </div>
                );
              })}
            </div>
          ...
          {tab === "circle" && (<div>
            {previewTag}
            <ActHeader
              eyebrow="Real connection, not broadcast"
              title="Truths that live between two people."
              lede="A circle pattern is jointly owned. Visible only to both of you, broadcastable by neither. No feed. No followers. Some patterns stay locked until the other person is here to see them too."
            />
            {!circleUnlocked ? (
              <div style={{ ...S.card, padding: 32, textAlign: "center" }}>
                ...
                <p style={{ fontSize: 13, color: T.ink2, lineHeight: 1.7, maxWidth: 380, margin: "0 auto" }}>Your circle opens when enough of you exists here to connect with. You can't share a self you haven't built. Keep writing.</p>
              </div>
            ) : (<>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10, margin: "0 0 24px" }}>
                {circle.map((m) => m.joined ? (
                  <div key={m.id} style={{ background: T.glass, borderRadius: 14, padding: "16px 12px", border: `1px solid ${T.border}`, textAlign: "center" }}>
                  ...
                  <div key={m.id} onClick={() => setInviteFor(m)} style={{ background: "rgba(10,13,20,0.4)", borderRadius: 14, padding: "16px 12px", border: "1.5px dashed rgba(90,100,114,0.35)", textAlign: "center", cursor: "pointer" }}>
                    <div style={{ width: 44, height: 44, borderRadius: "50%", border: `1.5px dashed ${T.ink4}`, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "center", color: T.ink4, fontSize: 18 }}>+</div>
              ...
              <div style={{ ...S.eyebrow, color: T.ink3, marginBottom: 12 }}>Circle patterns · {CIRCLE_PATTERNS.filter((p) => !p.locked).length} of {CIRCLE_PATTERNS.length} discovered</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {CIRCLE_PATTERNS.map((cp) => !cp.locked ? (
                  <div key={cp.id} style={{ ...S.card, marginBottom: 0, borderLeft: `3px solid ${T.green}`, padding: "18px 20px" }}>
                    <p style={{ margin: "0 0 12px", fontSize: 14.5, lineHeight: 1.75, fontFamily: T.serif, color: "#DAE2EC" }}>{cp.insight}</p>
                    <div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" }}>
                      {cp.people.map((p, i) => <span key={i} style={S.badge(p === "You" ? T.goldBright : T.blueBright)}>{p}</span>)}
                      <span style={{ marginLeft: "auto", fontSize: 10, padding: "3px 10px", borderRadius: 99, background: "rgba(84,196,142,0.12)", color: T.green }}>discovered together</span>
                    </div>
                  </div>
                ) : (
                  <div key={cp.id} style={{ ...S.card, marginBottom: 0, background: "rgba(14,18,25,0.35)", borderLeft: "3px solid rgba(90,100,114,0.5)", padding: "18px 20px" }}>
                    <p style={{ margin: "0 0 12px", fontSize: 14, lineHeight: 1.75, color: T.ink3 }}>{cp.teaser}</p>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                      {cp.people.map((p, i) => <span key={i} style={S.badge(T.ink4)}>{p}</span>)}
                      <button onClick={() => setInviteFor(circle.find((m) => m.id === cp.requires[0]))} style={{ marginLeft: "auto", padding: "6px 14px", borderRadius: 9, fontSize: 11, fontWeight: 600, cursor: "pointer", background: "rgba(201,169,110,0.1)", color: T.gold, border: "1px solid rgba(201,169,110,0.28)", fontFamily: "inherit" }}>🔒 Needs {cp.requires.map((r) => circle.find((m) => m.id === r)?.name).join(" + ")}</button>
                    </div>
                    <div style={{ fontSize: 10.5, color: "#39414D", marginTop: 9 }}>This pattern exists in the space between you. Neither of you can see it alone.</div>
                  </div>
                ))}
              </div>
            </>)}
          ...
          {tab === "dialogue" && (<div>
            {previewTag}
            <ActHeader
              eyebrow="Mediated courage ✦"
              title="The conversation you couldn't start."
              lede="Ask Maria's persona what you were afraid to ask her. It answers only from what she allowed, and she sees every question you ask. Vulnerability with the lights on. Nothing here is secret from her."
            />
            <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "0 0 20px", padding: 16, borderRadius: 14, background: T.glass, border: `1px solid ${T.border}` }}>
              <Glyph params={glyphParams("maria-spontaneity-warmth-art")} size={40} />
              <div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>Maria's persona</div>
                <div style={{ fontSize: 11, color: T.green }}>● mutual trust · she sees every question you ask</div>
              </div>
            </div>
            <div style={{ ...S.eyebrow, color: T.ink3, marginBottom: 12 }}>Ask what you can't say out loud</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {DIALOGUE_SUGGESTIONS.map((q, i) => (
                <button key={i} onClick={() => setDialogueQ(i)} style={{ display: "block", width: "100%", textAlign: "left", borderRadius: 11, padding: "13px 16px", fontSize: 13.5, fontFamily: T.serif, cursor: "pointer", lineHeight: 1.55, background: dialogueQ === i ? "rgba(14,18,25,0.7)" : "rgba(10,13,20,0.4)", border: dialogueQ === i ? "1px solid rgba(201,169,110,0.3)" : `1px solid ${T.borderSoft}`, color: dialogueQ === i ? T.ink : T.ink2, transition: "border-color .2s" }}>"{q}"</button>
              ))}
            </div>
            {dialogueQ !== null && (
              <div style={{ marginTop: 16, borderRadius: 14, background: T.glass, border: `1px solid ${T.border}`, borderLeft: `3px solid ${T.blueBright}`, padding: "18px 20px" }}>
                <div style={{ ...S.eyebrow, color: T.blueBright, marginBottom: 10 }}>Maria's persona responds</div>
                <p style={{ margin: 0, fontSize: 14, lineHeight: 1.85, color: "#C9D1D9", fontFamily: T.serif, minHeight: 48 }}>{dialogueShown}{dialogueTyping && <span style={{ opacity: 0.5 }}>▊</span>}</p>
                {!dialogueTyping && (
                  <div style={{ marginTop: 14, padding: "10px 14px", background: T.well, borderRadius: 10, fontSize: 11.5, color: T.green, lineHeight: 1.6, border: `1px solid ${T.borderSoft}` }}>✓ {DIALOGUE_RESPONSES[dialogueQ].note}</div>
                )}
              </div>
            )}
          </div>)}
          ...
          {tab === "portable" && (<div>
            <ActHeader
              eyebrow="Same you, different context"
              title="A self that can travel."
              lede="Because it is evidenced and consented-to, your persona can go where you can't, and say only what you allowed. Recipients see patterns, never raw entries. Choose the context."
            />
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(146px, 1fr))", gap: 9, margin: "0 0 22px" }}>
              {Object.entries(CONTEXTS).map(([k, c]) => (
                <div key={k} onClick={() => setPCtx(k)} style={{ textAlign: "left", padding: 14, borderRadius: 13, cursor: "pointer", background: pCtx === k ? "rgba(14,18,25,0.7)" : "rgba(10,13,20,0.4)", border: pCtx === k ? "1px solid rgba(201,169,110,0.3)" : `1px solid ${T.borderSoft}`, transition: "border-color .2s" }}>
                  <div style={{ fontSize: 19, marginBottom: 5, color: T.goldBright }}>{c.icon}</div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{c.label}</div>
                  <div style={{ fontSize: 10.5, color: T.ink2, marginTop: 2, lineHeight: 1.4 }}>{c.desc}</div>
                </div>
              ))}
            </div>
            <div style={{ ...S.card, padding: "22px 24px" }}>
              <div style={{ ...S.eyebrow }}>ThirdPersona · {CONTEXTS[pCtx].label}</div>
              <div style={{ ...S.display, fontSize: 20, margin: "8px 0 16px" }}>{CONTEXTS[pCtx].title}</div>
              {CONTEXTS[pCtx].insights.map((ins, i) => (
                <div key={i} style={{ padding: "12px 0", borderTop: i === 0 ? "none" : `1px solid ${T.borderSoft}`, fontSize: 13.5, lineHeight: 1.75, color: "#C9D1D9" }}>{ins}</div>
              ))}
              <div style={{ marginTop: 16, padding: "11px 15px", background: T.well, border: `1px solid ${T.borderSoft}`, borderRadius: 10, fontSize: 11.5, color: T.ink2, lineHeight: 1.6 }}>Derivative view. Recipients see patterns, never your raw entries. Revocable at any time.</div>
            </div>
          </div>)}
        </section>
      </div>
      ...
      {inviteFor && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(2,4,8,0.82)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }} onClick={() => setInviteFor(null)}>
          <div style={{ background: T.surface, borderRadius: 18, padding: 28, maxWidth: 420, width: "100%", border: `1px solid ${T.border}`, textAlign: "center", boxShadow: "0 30px 80px rgba(0,0,0,0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 13, color: T.ink2, lineHeight: 1.7, marginBottom: 6 }}>There are truths between you and {inviteFor.name} that neither of you can see alone.</p>
            <p style={{ fontSize: 12, color: T.ink4, lineHeight: 1.6, marginBottom: 22 }}>When {inviteFor.name} joins, those patterns become visible to both of you. Only to both of you.</p>
            <div style={{ background: T.well, borderRadius: 12, padding: "14px 16px", marginBottom: 20, textAlign: "left", border: `1px solid ${T.borderSoft}` }}>
              ...
              <p style={{ margin: 0, fontSize: 12.5, color: "#C6CFDA", lineHeight: 1.7, fontStyle: "italic", fontFamily: T.serif }}>"I've been building who I actually am, not who I perform. There's a part of it I can't see without you. No feeds, no followers, just us. Want in?"</p>
            </div>
            <button style={{ ...S.btnGold, width: "100%", padding: "13px", fontSize: 13, marginBottom: 8 }}>Send invitation to {inviteFor.name}</button>
            <button onClick={() => setInviteFor(null)} style={{ width: "100%", padding: "10px", background: "none", color: T.ink4, border: "none", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Not yet</button>
          </div>
        </div>
      )}
    </div>
